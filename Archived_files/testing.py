...
# Load the excel file
file_path = 'Nabers_book_testing.xlsx'
...
# Dictionary to store the R-squared scores and model information
scores = {}

# Evaluate each model and extract coefficients and intercept
for name, model in models.items():
    ...
    # For linear models, print the slope and intercept
...
def calculate_target_max_electricity(floor_m2, hours, star_rate):
    if isinstance(best_model, (LinearRegression, Ridge, Lasso)):
        ...
        # Linear equation calculation using the slope and intercept
        prediction = intercept + coefficients[0] * floor_m2 + coefficients[1] * hours + coefficients[2] * star_rate
        return prediction
    else:
        # For other models, just use their predict method
